# Runtime Feature Falsifier v2.9.5 — integration notes
...
uv tool install runtime-feature-falsifier-cli \
  --from git+https://github.com/<owner>/<repo>.git@v2.9.5
...
rff self upgrade --from git+https://github.com/<owner>/<repo>.git@v2.9.5
```
...
Use `rff audit ...` for audit state operations. Audit output roots are configurable through `.rff.toml`; agents must resolve active state with `rff audit where` rather than assuming `.runtime-feature-audit`. Canonical results are generated by `rff audit report`, sealed by `rff audit gate`, and presented through `rff audit present`.
